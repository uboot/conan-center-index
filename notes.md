...
- `ffmpeg` requires `freetype/2.13.2`. This conflicts with thez version range
  `freetype/[>=2.13.2 <3]` which is required by e.g. `fontconfig`. This is
  currently fixed by patching the `ffmpeg` recipe
- Move the options to a separate profile such that it can be included last. This
  make sure that the package specific options override the general options. E.g.
  `onnx/*:shared=False` will override `*:shared=True`